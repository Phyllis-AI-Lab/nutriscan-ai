      ...
      <div className="w-full max-w-md bg-white/80 backdrop-blur-xl rounded-[40px] shadow-[0_30px_60px_-15px_rgba(0,0,0,0.1)] border border-white/60 relative overflow-hidden min-h-[85vh] flex flex-col">
        ...
        <div className="px-6 py-5 bg-white/50 backdrop-blur-md sticky top-0 z-20 border-b border-white/50">
          <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={() => setView(view === 'today' ? 'history' : 'today')} className={`p-2 rounded-full transition-colors ${view === 'history' ?
              'bg-emerald-100 text-emerald-600' : 'text-slate-400 hover:bg-slate-100'}`}>
            ...
            <div className="space-y-4 animate-fade-in-up">
              ...
              {history.length === 0 ? <div className="text-center py-10 text-slate-300 text-sm">暫無歷史紀錄</div> : visibleHistory.map((day) => (
                <div key={day.id} onClick={() => toggleHistoryItem(day.id)} className={`bg-white rounded-2xl border transition-all cursor-pointer overflow-hidden ${expandedDayId === day.id ?
                  'border-emerald-200 shadow-md ring-1 ring-emerald-100' : 'border-slate-100 shadow-sm hover:border-emerald-100'}`}>
              ...
              {foodLog.length > 0 && (
                <div className="space-y-4">
                  ...
                  {foodLog.slice().reverse().map((item) => (
                    <div key={item.id} className={`bg-white rounded-2xl p-4 shadow-sm border border-slate-100 flex items-center justify-between group transition-all ${result?.id === item.id ?
        ...
        {result && view === 'today' && (
          <div className="fixed bottom-8 left-1/2 -translate-x-1/2 w-full max-w-[calc(100%-3rem)] sm:max-w-xs z-50 animate-fade-in-up">
            <button onClick={() => {
              setResult(null); setImages([]); window.scrollTo({ top: 0, behavior: 'smooth' });
            }} className="w-full bg-slate-900 text-white h-14 rounded-full font-bold shadow-2xl flex items-center justify-center gap-2">
