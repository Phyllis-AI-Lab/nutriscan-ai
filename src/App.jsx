...
import { Camera, Leaf, Info, RotateCcw, Edit3, Check, Image as ImageIcon, Sparkles, Activity, ShieldAlert, Trash2, Clock, CalendarDays, ChevronDown, ChevronUp, RefreshCw, ArrowLeft, Save, Archive, ArrowUpFromLine } from 'lucide-react';
...
function App() {
  ...
  const deleteHistoryItem = (e, id) => {
    e.stopPropagation(); // 防止觸發摺疊/展開
    if (window.confirm('確定要永久刪除這天的紀錄嗎？此操作無法復原。')) {
  ...
  const analyzeFood = async (file) => {
    ...
    try {
      ...
      const prompt = `你是一位專業的臨床營養師。請分析這張食物照片。
      【用戶背景】
      ...
      else throw new Error("AI 回傳格式錯誤");
      const data = JSON.parse(jsonString);
      const newRecord = { ...data, id: Date.now(), timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }), imgUrl: URL.createObjectURL(file) };
      setResult(newRecord); setFoodLog(prev => [...prev, newRecord]); setCurrentFile(null);
      setTimeout(() => resultRef.current?.scrollIntoView({ behavior: "smooth" }), 100);
    } catch (error) {
      console.error(error);
      let msg = error.message || "";
      if (msg.includes("404")) setErrorMsg(`模型設定錯誤 (404)，請檢查 API Key。`);
      else if (msg.includes("JSON")) setErrorMsg("資料解析失敗，請點擊下方按鈕重試");
      else if (msg.includes("429")) setErrorMsg("請求太頻繁，請稍候再試");
  ...
  const handleFileSelect = (e) => {
    ...
    if (file) { setImage(URL.createObjectURL(file)); setCurrentFile(file); analyzeFood(file); }
    e.target.value = '';
  };
  const handleRetry = () => { if (currentFile) analyzeFood(currentFile); };
      ...
      <div className="w-full max-w-md bg-white/80 backdrop-blur-xl rounded-[40px] shadow-[0_30px_60px_-15px_rgba(0,0,0,0.1)] border border-white/60 relative overflow-hidden min-h-[85vh] flex flex-col">
                  ...
                  {isEditingLimit ? (
                    <div className="flex items-center gap-2 animate-fade-in-up">
                      <input 
                        ...
                        className="w-20 bg-white border border-emerald-200 rounded-xl px-2 py-1.5 text-center text-lg text-emerald-600 focus:outline-none focus:ring-2 focus:ring-emerald-400 shadow-sm" 
                        autoFocus 
                      />
                      <button onClick={() => setIsEditingLimit(false)} className="p-2 bg-emerald-500 text-white rounded-xl shadow-md hover:bg-emerald-600 active:scale-95 transition-all">
                        <Check size={18} />
                      </button>
                    </div>
                  ) : (
          ...
          {view === 'history' ? (
            <div className="space-y-4 animate-fade-in-up">
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest pl-2">
                History Log {showAllHistory ? '(全部)' : `(近${SHOW_LIMIT}筆)`}
              </h3>
              {history.length === 0 ? (
                <div className="text-center py-10 text-slate-300 text-sm">暫無歷史紀錄<br/>請先完成今日記錄並按下「結算」</div>
              ) : (
                <>
                  {visibleHistory.map((day) => {
                    const isExpanded = expandedDayId === day.id;
                    return (
                      <div key={day.id} onClick={() => toggleHistoryItem(day.id)} className={`bg-white rounded-2xl border transition-all cursor-pointer overflow-hidden ${isExpanded ? 'border-emerald-200 shadow-md ring-1 ring-emerald-100' : 'border-slate-100 shadow-sm hover:border-emerald-100'}`}>
                        <div className="p-5 flex justify-between items-center bg-white relative z-10">
                          <div className="flex items-center gap-3">
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white text-xs font-bold ${day.isOverLimit ? 'bg-rose-400' : 'bg-emerald-400'}`}>
                              {Math.round((day.totalCalories / day.limit) * 100)}%
                            </div>
                            <div>
                              <div className="flex items-center gap-2">
                                 <span className="font-bold text-slate-700">{day.date}</span>
                                 <span className="text-[10px] text-slate-400 font-medium bg-slate-50 px-2 py-0.5 rounded-full">{day.totalCalories} kcal</span>
                              </div>
                              <div className="text-[10px] text-slate-400 mt-0.5">{isExpanded ? '點擊收合詳情' : `吃了 ${day.foodList.length} 餐...`}</div>
                            </div>
                          </div>
                          <div className="text-slate-300">{isExpanded ? <ChevronUp size={20} /> : <ChevronDown size={20} />}</div>
                        </div>
                        <div className={`transition-[max-height] duration-300 ease-in-out bg-slate-50/50 ${isExpanded ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'}`}>
                          <div className="p-5 pt-0 border-t border-slate-100">
                             <div className="mt-3 space-y-2">
                                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">攝取清單</p>
                                <p className="text-sm text-slate-700 leading-relaxed">{day.foodList.join("、")}</p>
                             </div>
                             <div className="mt-4 flex gap-4 text-[10px]">
                                <span className={getNutrientColor(day.nutrients.p, day.goals?.protein, 'p')}>蛋 {day.nutrients.p} <span className="text-slate-300">/ {day.goals?.protein || '-'}g</span></span>
                                <span className={getNutrientColor(day.nutrients.f, day.goals?.fat, 'f')}>脂 {day.nutrients.f} <span className="text-slate-300">/ {day.goals?.fat || '-'}g</span></span>
                                <span className={getNutrientColor(day.nutrients.c, day.goals?.carbs, 'c')}>碳 {day.nutrients.c} <span className="text-slate-300">/ {day.goals?.carbs || '-'}g</span></span>
                             </div>
                             
                             {/* 🔥 V9.6 新增：刪除按鈕 (位於卡片展開後的底部) */}
                             <div className="flex justify-end mt-4 pt-3 border-t border-slate-200/50">
                                <button 
                                  onClick={(e) => deleteHistoryItem(e, day.id)}
                                  className="flex items-center gap-1.5 text-[11px] font-bold text-rose-400 hover:text-rose-600 hover:bg-rose-50 px-3 py-1.5 rounded-lg transition-colors"
                                >
                                  <Trash2 size={14} /> 刪除紀錄
                                </button>
                             </div>
                  ...
                  {history.length > SHOW_LIMIT && (
                    !showAllHistory ? (
                      <button onClick={() => setShowAllHistory(true)} className="w-full py-3 text-xs font-bold text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-xl transition-colors flex items-center justify-center gap-2">
                        <Archive size={14} /> 查看更早的紀錄 ({history.length - SHOW_LIMIT} 筆)
                      </button>
                    ) : (
                      <button onClick={() => setShowAllHistory(false)} className="w-full py-3 text-xs font-bold text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-xl transition-colors flex items-center justify-center gap-2">
                        <ArrowUpFromLine size={14} /> 收合清單 (只看最近{SHOW_LIMIT}筆)
                      </button>
                    )
                  )}
                </>
            ...
            <>
              <div className="relative w-full aspect-[4/3] bg-gradient-to-br from-slate-50 to-slate-100 rounded-[32px] overflow-hidden border-4 border-white shadow-2xl shadow-slate-200/50 group">
                {image ? (
                  ...
                  <div className="w-full h-full flex flex-col items-center justify-center gap-5">
                    <button onClick={() => cameraInputRef.current.click()} className="flex flex-col items-center gap-2 group/btn relative">
                      ...
                      <span className="text-sm font-bold text-slate-600">拍攝下一餐</span>
                    </button>
                    ...
                    <button onClick={() => uploadInputRef.current.click()} className="flex flex-col items-center gap-2 group/btn">
                      ...
                      <span className="text-xs font-bold text-slate-400">從相簿選擇</span>
                    </button>
                  </div>
                )}
                {loading && (
                  <div className="absolute inset-0 bg-white/80 backdrop-blur-sm flex flex-col items-center justify-center z-20">
                    ...
                    <p className="text-emerald-600 text-xs font-bold animate-pulse tracking-widest">AI ANALYSIS</p>
                  </div>
                )}
              </div>
              ...
              {errorMsg && (
                <div className="bg-rose-50 border border-rose-100 p-4 rounded-2xl animate-shake shadow-sm">
                  <div className="flex gap-3 items-center text-rose-500 text-xs font-bold mb-2"><Info size={16} /><span className="flex-1">{errorMsg}</span></div>
                  <button onClick={handleRetry} className="w-full bg-white border border-rose-200 text-rose-500 py-2 rounded-xl text-xs font-bold flex items-center justify-center gap-2 hover:bg-rose-50 transition-colors"><RefreshCw size={14} /> 點擊此處重試</button>
                </div>
              )}
                ...
                {result && (
                  <div className="space-y-5 animate-fade-in-up py-4 border-t border-dashed border-slate-200 mt-4">
                    ...
                    <div className="bg-slate-50 rounded-xl p-4 border border-slate-100">
                        <h4 className="text-xs font-bold text-slate-400 uppercase mb-2">食用份量建議</h4>
          ...
          <div className="pt-8 border-t border-slate-200/50">
            <div className="flex gap-3 items-start opacity-70 px-2">
                ...
                <p className="text-[10px] text-slate-400 leading-relaxed text-justify">
                    <span className="font-bold">免責聲明：</span>本應用程式僅提供 AI 輔助之熱量與飲食建議，<span className="font-bold">並非醫療診斷</span>。數據與脂肪肝風險評估可能存在誤差。患有慢性疾病者，請務必諮詢專業醫師或營養師的個別化指導。
                </p>
            </div>
          </div>
        </div>
        ...
        {result && view === 'today' && (
          <div className="fixed bottom-8 left-1/2 -translate-x-1/2 w-full max-w-[calc(100%-3rem)] sm:max-w-xs z-50 animate-fade-in-up">
            <button onClick={() => { setResult(null); setImage(null); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className="w-full bg-slate-900 text-white h-14 rounded-full font-bold shadow-2xl flex items-center justify-center gap-2">
